...
function App() {
	...
	function MenuAfterGame(_action) {
		setMenu(true);
	}
	
	function NextQuest(_action) {
		setNext(true);
	}
	...
	function chooseTheme(action) {
		...
		} else if (action.id === '2' || action.id === 'Животные' || action.id ===
			'животные') {
			const res = scale.map((o, index) => {
				return { ...o, status: false, id: index };
			});
	...
	function closeModalWindow(_action) {
		setModalState(false);
		setStateModalQuiz(false);
	}
